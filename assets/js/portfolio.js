...
$(document).ready(function(){
    ...
    function openLightbox($item) {
        const src = $item.attr('data-large') || $item.find('img').attr('src');
        const title = $item.find('h3').text() || '';
        const desc = $item.find('.project-desc').text() || '';

        $lightboxImg.attr('src', src);
        $lightboxCaption.text(title + (desc ? ' — ' + desc : ''));
    ...
    // Open when clicking preview buttons or image
    $(document).on('click', '.btn-lightbox, .project-media img', function(e){
        e.preventDefault();
...
$(document).ready(function(){
    ...
    $form.on('submit', function(e){
        ...
        const mailtoSubject = encodeURIComponent('Contact: ' + subject);
        const mailtoBody = encodeURIComponent(`Name: ${name}\nEmail: ${email}\n\n${message}`);
        const mailtoLink = `mailto:mirofrontend@outlook.com?subject=${mailtoSubject}&body=${mailtoBody}`;

        // Open user's default email client
        window.location.href = mailtoLink;

        // Show success feedback
        $status.text('✓ Opening your mail client...');
        $status.css('color', '#bfe5c8');
